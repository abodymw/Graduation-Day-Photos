...
- Photos are stored as raw files in Firebase Storage (a plain file bucket, not an image CDN) — no resizing or recompression ever happens, so downloads are byte-for-byte identical to what was uploaded.
